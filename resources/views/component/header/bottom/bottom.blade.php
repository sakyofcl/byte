                ...
                <div class="categories_wrap">
                    ...
                    <div id="navCatContent" class="nav_cat navbar collapse">
                        ...
                        <ul>
                            ...
                            ?>
                            @if ($main !== 0)
                            @for ($i = 0; $i < $main; $i++) @if ($i <=9) <li class="dropdown dropdown-mega-menu">
                                <a class="dropdown-item nav-link dropdown-toggler text-uppercase" href="#" data-toggle="dropdown">
                                            ...
                                            <ul class="d-lg-flex">
                                                <li class="mega-menu-col col-lg-6">
                                                    <ul>
                                                        ...
                                                        @if ($sub !==0)
                                                        @for ($s = 0; $s < $sub; $s++) @if ($maindata[$i]->catid == $subdata[$s]->catid)
                                                            <li>
                                                                <a class="dropdown-item nav-link nav_item text-uppercase" href="/product/{{$maindata[$i]->name}}/{{ $subdata[$s]->name}}/{{ $subdata[$s]->subid }}">{{ $subdata[$s]->name }}
                                                                </a>
                                                            </li> @endif
                                                            @endfor
                                                            @endif
                                ...
                                <li>
                                    <ul class="more_slide_open">
                                        <li class="dropdown dropdown-mega-menu">
                                            <a class="dropdown-item nav-link dropdown-toggler" href="#" data-toggle="dropdown">
                                                <i class="ti-view-grid text-danger" style="font-size:13px;"></i>
                                                <span>{{ $maindata[$i]->name }}</span>
                                            </a>
                                                ...
                                                <ul class="mega-menu d-lg-flex">
                                                    <li class="mega-menu-col col-lg-7">
                                                        <ul class="d-lg-flex">
                                                            <li class="mega-menu-col col-lg-6">
                                                                <ul>
                                                                    <a href="product/{{ $maindata[$i]->name }}">
                                                                        <li class="dropdown-header">
                                                                            {{ $maindata[$i]->name }}
                                                                        </li>
                                                                    </a>
                                                                    @if ($sub !== 0)
                                                                    @for ($s = 0; $s < $sub; $s++) @if ($maindata[$i]->catid ==
                                                                        $subdata[$s]->catid) <li>
                                                                            <a class="dropdown-item nav-link nav_item" href="/product/{{$maindata[$i]->name}}/{{ $subdata[$s]->name}}/{{ $subdata[$s]->subid }}">{{ $subdata[$s]->name }}
                                                                            </a>
                                                                        </li> @endif
                                                                        @endfor
                                                                        @endif
